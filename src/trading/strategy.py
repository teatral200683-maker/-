...
class TradingStrategy:
    ...
    async def _check_trailing_tp(self, price: float):
        """
        Проверка Trailing Take Profit.

        Логика:
        1. Цена достигает avg + activation_pct% → активируем trailing
        2. Отслеживаем максимум цены (_trailing_high)
        3. Если цена откатилась от максимума на callback_pct% → закрываем
        """
        ...
        # Шаг 2: Обновляем максимум
        if price > self._trailing_high:
            self._trailing_high = price

        # Шаг 3: Проверяем откат
    ...
    def _should_first_entry(self, current_price: float) -> bool:
        ...
        if drop_pct >= self.entry_step_pct:
            # Фильтр тренда: не входить при сильном падении (RSI слишком низкий)
