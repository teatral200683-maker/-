...
class Indicators:
    ...
    Накапливает ценовые данные и предоставляет:
    ...
    def __init__(self, atr_period: int = 14, ema_period: int = 50, rsi_period: int = 14):
        ...
        # Счётчик тиков для обновления раз в N тиков
        self._tick_count: int = 0
        self._update_interval: int = 60  # Пересчёт каждые 60 тиков (~1 мин)
    ...
    def update(self, price: float):
        """
        Обновить индикаторы новой ценой.

        На каждом тике обновляем текущую свечу.
        Раз в _candle_size тиков — закрываем свечу и пересчитываем индикаторы.
        """
        self._tick_count += 1
    ...
    def _calc_ema(self, prices: list, period: int) -> Optional[float]:
        ...
        EMA = price * k + EMA_prev * (1-k), где k = 2/(period+1)
        """
        if len(prices) < period:
            return None

        k = 2 / (period + 1)
        ema = sum(prices[:period]) / period  # SMA для стартовой точки
    ...
    def _calc_rsi(self, prices: list, period: int) -> Optional[float]:
        ...
        # Средний рост и падение за последние period
    ...
    def get_atr(self) -> Optional[float]:
        """Получить текущий ATR (None если мало данных)."""
        return self._last_atr

    def get_ema(self) -> Optional[float]:
        """Получить текущий EMA (None если мало данных)."""
        return self._last_ema

    def get_rsi(self) -> Optional[float]:
        """Получить текущий RSI 0-100 (None если мало данных)."""
        return self._last_rsi

    def get_volatility_factor(self, base_atr: float = 50.0) -> float:
        """
        Коэффициент волатильности для адаптивного размера позиции.

        Возвращает множитель 0.3–1.5:
        - ATR низкий → коэффициент > 1.0 (увеличить позицию)
        - ATR высокий → коэффициент < 1.0 (уменьшить позицию)

        Args:
            base_atr: Нормальный ATR для данного актива (по умолчанию $50 для ETH)
        ...
        if atr is None or atr <= 0:
            return 1.0  # Нет данных → без изменений

        factor = base_atr / atr
        # Ограничиваем: не менее 0.3, не более 1.5
